...
# Fuzzy Sugeno
# Studi Kasus : Penjualan Beras Bulog

# Permintaan Barang : min 50 karung, max 100 karung.
...
class Penjualan():
    ...
    def inferensi(self, jml_request, jml_sedia):
        result = []
        rq = Permintaan()
        rs = Persediaan()
        ...
        α1 = min(rq.turun(jml_request), rs.banyak(jml_sedia))
        z1 = self.kurang(α1)
        result.append((α1, z1))

        # [R2] JIKA Permintaan TURUN, dan Persediaan SEDIKIT, 
        #     MAKA Penjualan Barang BERKURANG
        α2 = min(rq.turun(jml_request), rs.sedikit(jml_sedia))
        z2 = self.kurang(α2)
        result.append((α2, z2))

        # [R3] JIKA Permintaan NAIK, dan Persediaan BANYAK, 
        #     MAKA Penjualan Barang BERTAMBAH
        α3 = min(rq.naik(jml_request), rs.banyak(jml_sedia))
        z3 = self.tambah(α3)
        result.append((α3, z3))

        # [R4] JIKA Permintaan NAIK, dan Persediaan SEDIKIT,
        #     MAKA Penjualan Barang BERTAMBAH * 2
        α4 = min(rq.naik(jml_request), rs.sedikit(jml_sedia))
        z4 = self.tambah(α4) * 2
        result.append((α4, z4))

        # [R5] JIKA Permintaan NAIK, dan Persediaan CUKUP,
        #     MAKA Penjualan Barang Kurang / 2.
        α5 = min(rq.naik(jml_request), rs.cukup(jml_sedia))
        z5 = self.kurang(α5) / 2
        result.append((α5, z5))

        # [R6] JIKA Permintaan NAIK, dan Persediaan CUKUP,
        #     MAKA Penjualan Barang BERTAMBAH.
        α6 = min(rq.turun(jml_request), rs.cukup(jml_sedia))
        z6 = self.tambah(α6)
        result.append((α6, z6))

        return result

    def defuzifikasi(self, jml_request, jml_sedia):
        inferensi_values = self.inferensi(jml_request, jml_sedia)
